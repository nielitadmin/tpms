...
// Include Database connection to get real TP counts
require_once 'includes/config.php';

// Fetch dynamic TP counts per district
$districtTPCounts = [];
try {
    // Query to count approved centers grouped by district
    // Note: Make sure your 'centers' table has a 'district' column!
    $stmt = $pdo->query("SELECT district, COUNT(*) as count FROM centers WHERE status = 'Approved' AND district IS NOT NULL GROUP BY district");
    while ($row = $stmt->fetch()) {
        // We make the key lowercase to easily match it with the map data later
        $districtTPCounts[strtolower(trim($row['district']))] = $row['count'];
    }
} catch (PDOException $e) {
    // FALLBACK: If your table doesn't have a district column yet, use this dummy data to test the UI!
    $districtTPCounts = [
        'khordha' => 18, 'cuttack' => 12, 'ganjam' => 8, 'sundargarh' => 5, 
        'raipur' => 22, 'bilaspur' => 14, 'durg' => 9, 'bastar' => 4
    ];
}
// Convert PHP array to a JSON object for JavaScript to read
                        ...
                        .on("mouseover", function(event, d) {
                            ...
                            const tooltipContent = `
                                ...
                                <div style="font-size: 12px; color: #cbd5e1; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 6px;">
                                    Active Centers: <span style="color: #4ade80; font-weight: 800; font-size: 14px;">${activeTPs}</span>
                                </div>
                            `;
